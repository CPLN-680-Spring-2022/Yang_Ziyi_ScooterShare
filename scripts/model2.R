...
model1 <- glm(origins_cnt ~ ., family="poisson", data = as.data.frame(train.set))

# Predicting on train set
train.set$pred_rf <- predict(model1, train.set, type = "response")
...
test.set$pred_rf <- predict(model1, test.set, type = "response")
...
model1 <- glm(origins_cnt ~ ., family="poisson", data = as.data.frame(train.set) %>% dplyr::select(osm_features, origins_cnt))
# Predicting on train set
train.set$pred_ps <- predict(model1, train.set, type = "response")
train.set$AE_ps <- abs(train.set$pred_ps - train.set$origins_cnt)
train.set$Error_ps <- train.set$pred_ps - train.set$origins_cnt
mean(train.set$AE_ps)
mean(train.set$origins_cnt)

test.set$pred_ps <- predict(model1, test.set, type = "response")
test.set$AE_ps <- abs(test.set$pred_ps - test.set$origins_cnt)
test.set$Error_ps <- test.set$pred_ps - test.set$origins_cnt
mean(test.set$AE_ps)
mean(test.set$origins_cnt)
